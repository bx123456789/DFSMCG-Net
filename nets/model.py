...
class SupervisedAttentionModule(nn.Module):
    def __init__(self, mid_d):
        ...
        self.mid_d = mid_d
        # fusion
        self.cls = nn.Conv2d(self.mid_d, 7, kernel_size=1)
        self.conv_context = nn.Sequential(
            nn.Conv2d(14, self.mid_d, kernel_size=1),
            nn.BatchNorm2d(self.mid_d),
            nn.ReLU(inplace=True)
        )
...
if __name__ == '__main__':
    x1 = torch.randn(1, 3, 512, 512)  # Assuming input channels are 3 and size is 224x224
    x2 = torch.randn(1, 3, 512, 512)
    model=BaseNet()
    mask_p2, mask_p3, mask_p4, mask_p5 = model(x1, x2)
    # Output shape check
    print("Mask p2 shape:", mask_p2.shape)
    print("Mask p3 shape:", mask_p3.shape)
    print("Mask p4 shape:", mask_p4.shape)
    print("Mask p5 shape:", mask_p5.shape)