...
sys.path.append('./')
from nets.resnet import resnet18, resnet34, resnet50
from nets.help_funcs import TwoLayerConv2d
...
if __name__ == '__main__':
    model = SFBIN(backbone='resnet18', output_sigmoid=False)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = model.to(device)

    # 批次大小
    batch_size = 1  # 每次推理的图片数量，可以根据需要调整
    num_images = 2048  # 总图片数量
    ...
    for i in range(0, num_images, batch_size):
        # 生成当前批次的张量
        batch_input1 = torch.randn(batch_size, 3, 256, 256, device=device)
        batch_input2 = torch.randn(batch_size, 3, 256, 256, device=device)

        # 执行推理
        out = model(batch_input1, batch_input2)
    ...
    # 计算每张图片的平均推理时间
    average_inference_time_per_image = total_inference_time / num_images  # 2048张图片
    print(f"Total Inference Time for 2048 images: {total_inference_time:.4f} seconds")
