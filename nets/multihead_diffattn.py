...
if __name__=="main":
    ...
    # 调整为 (bsz, tgt_len, embed_dim)
    x = x.view(4, 128, 256 * 256)  # x.shape becomes (4, 128, 65536)

    # 生成随机的相对位置编码，假设为两个 (128, 128) 的张量
    rel_pos = (torch.randn(128, 128), torch.randn(128, 128))

    # 初始化模型
    args = Args(model_parallel_size=1)
    embed_dim = 256 * 256  # Set to 65536 to match the reshaped tensor
    depth = 1
    num_heads = 8  # Number of attention heads
    model = MultiheadDiffAttn(args, embed_dim, depth, num_heads)
