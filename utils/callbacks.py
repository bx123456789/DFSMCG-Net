...
class EvalCallback():
    ...
    def get_miou_png(self, image_A,image_B):
        ...
        orininal_h  = np.array(image_A).shape[0]
        orininal_w  = np.array(image_A).shape[1]
        #---------------------------------------------------------#
        #   给图像增加灰条，实现不失真的resize
        #   也可以直接resize进行识别
        #---------------------------------------------------------#
        image_A, nw, nh  = resize_image(image_A, (self.input_shape[1],self.input_shape[0]))
        image_B, nw, nh  = resize_image(image_B, (self.input_shape[1],self.input_shape[0]))
        #---------------------------------------------------------#
        #   添加上batch_size维度
        #---------------------------------------------------------#
        # image_data_A  = np.expand_dims(np.transpose(preprocess_input(np.array(image_A, np.float32)), (2, 0, 1)), 0)
        # image_data_B  = np.expand_dims(np.transpose(preprocess_input(np.array(image_B, np.float32)), (2, 0, 1)), 0)
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                #   从文件中读取图像
                #-------------------------------#
                image_path_A  = os.path.join(self.dataset_path, "VOC2007/JPEGImages/A/"+image_id+".png")
                image_path_B  = os.path.join(self.dataset_path, "VOC2007/JPEGImages/B/"+image_id+".png")
                image_A       = Image.open(image_path_A)
                image_B       = Image.open(image_path_B)
                #------------------------------#
                #   获得预测txt
                #------------------------------#
                image       = self.get_miou_png(image_A,image_B)
